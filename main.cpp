...
void printfiles(const std::vector<size_t> &indices, const std::vector<std::string> &files, size_t from, size_t to, size_t selected);
...
int main(int argc, char *argv[])
{
	...
	const std::vector<std::string> exception_list = File::readfile(globals.paths.exception_list);
	...
	try
	{
		std::vector<std::string> contents = exception_list.empty() ? File::listfiles_recursive(home_folder) : File::listfiles_recursive(home_folder, exception_list);
	...
	auto refreshFiles = [&](std::vector<std::string> &f_names, const std::vector<std::string> &paths) {
		f_names.clear();
	...
	size_t from = 0;
	size_t to = std::min((size_t)globals.columns, show_files_indices.size());
	size_t selected = 0;
	bool running = true;
	...
	while (running)
	{
		...
		if (globals.ui_state == UI_State::MAIN_MENU)
		{
			...
			if (key == "\033[A" || key == "k")
			{
				...
				else
				{
					...
					from = (to > (size_t)globals.columns) ? to - globals.columns : 0;
				}
			}
			// NAVIGATION: Down
			else if (key == "\033[B" || key == "m")
			{
				...
				else
				{
					selected = 0;
					from = 0;
					to = std::min((size_t)globals.columns, show_files_indices.size());
				}
			}
			// NAVIGATION: Page Up (j)
			else if (key == "\033[D" || key == "j")
			{
				size_t step = (size_t)globals.columns;
			...
			else if (key == "\n" || key == " " || key == "/")
			{
				...
				if (File::isfile(fp))
				{
					...
					File::insertline(globals.paths.history, fp + globals.delimiter + funcs::currentTime(), 0);

					if (isMobileDevice && isVideoFile(fp) && launch_in_mpv)
					{
						const std::string command = "am start -a android.intent.action.VIEW -d \"file://" + fp + "\" -n is.xyz.mpv/.MPVActivity";
			...
			else if (key == "s" || (key == "S" && !isMobileDevice))
			{
				...
				{
					if (funcs::hasSequence(funcs::lowercase(full_paths[i]), search_string))
					{
			...
			{
				full_paths.clear();
				const std::vector<std::string> contents = exception_list.empty() ? File::listfiles_recursive(home_folder) : File::listfiles_recursive(home_folder, exception_list);
		...
		{
			std::vector<std::string> latest = File::readfile(globals.paths.latest_media);
			if (!latest.empty())
			{
				full_paths.clear();
...
void printfiles(const std::vector<size_t> &indices, const std::vector<std::string> &files, size_t from, size_t to, size_t selected)
{
	...
	std::cout << TXT_CYAN << "Select media " << _BOLD << selected + 1 << " / " << indices.size() << ":" << _RESET << std::endl;
	std::cout << TXT_BLUE << border << _RESET << std::endl;

	for (size_t i = from; i < to; i++)
	{
		size_t file_index = indices[i];
		std::string display_name = trimStr(files[file_index], funcs::str(i + 1).length());
		if (i == selected)
		{
			std::cout << _BOLD << TXT_YELLOW << i + 1 << _RESET << " "
					  << _BOLD << color::bg_rgb(80, 80, 80) << color::fg_rgb(120, 255, 120) << display_name << _RESET << std::endl;
		}
		else
		{
			std::cout << TXT_YELLOW << i + 1 << _RESET << " " << display_name << std::endl;
		}
	}
